...
layouts = {'QWERTY': (QUERTY_KEYBOARD, QWERTY_SHIFTED_KEYBOARD)}

# Sets the default keyboard to use to be QWERTY.
keyboard = QUERTY_KEYBOARD
shifted_keyboard = QWERTY_SHIFTED_KEYBOARD
...
def array_for_char(c):
    if (True in [c in r for r in keyboard]):
        return keyboard
    elif (True in [c in r for r in shifted_keyboard]):
...
def euclidean_keyboard_distance(c1, c2):
    coord1 = get_character_coord(c1, array_for_char(c1))
    coord2 = get_character_coord(c2, array_for_char(c2))
...
def insertion_cost(s, i, c):
    ...
    cost = INSERTION_COST
    if array_for_char(s[i]) != array_for_char(c):
...
def typo_distance(s, t, layout='QWERTY'):
    ...
    for i in range(len(t) + 1):
        ...
        cost = 0.0
        for j in range(i):
            cost += insertion_cost(intermediateString, j - 1, t[j - 1])
    ...
    for j in range(1, len(t) + 1):
        for i in range(1, len(s) + 1):
            if s[i - 1] == t[j - 1]:
                ...
                insertCost = insertion_cost(s, i, t[j - 1])
                subCost = substitution_cost(s, i - 1, t[j - 1])
...
def get_possible_actions(s, layout='QWERTY'):
    ...
    for i in range(len(s)):
        ...
        for key in (sum([r for r in keyboard], []) + sum([r for r in shifted_keyboard], [])):
...
def typo_generator(s, d, layout='QWERTY'):
    t = 0
    r = d
    actions = get_possible_actions(s, layout)
